...
- [ ] **Rubric gap fixes** (Decision 6): Update the evaluation rubric with gaps discovered during PRD #2 — CDQ-008 (tracer naming consistency), RST-004 refinement (I/O boundary exception), CDQ-007 update (conditional attribute setting). Do this before the spec update because the spec references the updated rubric.
