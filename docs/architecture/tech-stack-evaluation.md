...
**Node.js 24.x includes native TypeScript type stripping**, enabled by default with no flags required. The runtime strips type annotations at load time, allowing direct `node src/index.ts` execution. Limitations: only erasable syntax is supported (no `enum`, no `namespace` with runtime code, no parameter properties, no import aliases, no decorators). TypeScript 5.8's `erasableSyntaxOnly` tsconfig option enforces these constraints at compile time. `tsc --noEmit` serves as the CI type-checking gate — the runtime provides execution, not type safety.
